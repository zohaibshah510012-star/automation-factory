import { getAiProviders, getActiveProviderName, getImageProvider, getImageProviderName, getVideoProvider, getVideoProviderName } from "@/lib/ai-providers";
...
export async function generateVideo(input:{taskId:string;prompt:string;model?:string;durationSeconds?:number}){const video=await getVideoProvider().generateVideo(input);return {provider:getVideoProviderName(),video};}
