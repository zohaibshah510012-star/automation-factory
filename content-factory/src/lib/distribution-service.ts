import { MockDistributionProvider } from "@/lib/distribution-provider";
...
export async function processDistributionJob(id: string) {
  ...
  try {
    const result = await new MockDistributionProvider().publish({ platform: job.platform, payload: job.payload });
