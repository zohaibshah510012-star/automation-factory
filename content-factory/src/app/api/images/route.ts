...
export async function POST(request: Request) { try { const user = await requireUser(request); const body = await request.json() as { prompt?: string; model?: string; size?: string }; const prompt = body.prompt?.trim(); if (!prompt) return NextResponse.json({ error: "Image prompt is required" }, { status: 400 }); assertImageProviderConfigured(); await assertDailyGenerationLimit(user.id); const task = await createImageTask({ userId: user.id, prompt, model: body.model, size: body.size }); await trackProductEvent({ eventName: "task_create", userId: user.id, surface: "image", path: "/api/images", properties: { taskId: task.id, taskType: "image" } }); await trackGenerationStartedMilestone({ userId: user.id, taskId: task.id, taskType: "image", surface: "image", path: "/api/images" }); void runImageTask(task.id); return NextResponse.json({ task }, { status: 201 }); } catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to create image task" }, { status: 400 }); } }
